...
node packages/codegraph/dist/cli.js migrate-learnings [.]  # Migrate learnings.md → Memory Graph
...
<!-- memorygraph:start -->
# Memory Graph — Typed Knowledge System

The Memory Graph stores learnings, facts, decisions, and patterns in a typed SQLite graph with relationships. It replaces flat `learnings.md` files with structured, searchable, relational memory.
...
| `Updates` | New memory supersedes old |
...
| Tool | When to use |
|------|-------------|
| `memory_add` | Capture a new learning/fact/decision |
| `memory_search` | Find memories by natural language query (FTS) |
| `memory_query` | Filter by type, project, skill, confidence, tags |
| `memory_load` | Load top-scored memories for session (replaces load-learnings) |
| `memory_add_edge` | Create relationship between two memories |
| `memory_stats` | View Memory Graph statistics and contradictions |
| `memory_decay` | Apply decay cycle (post-session) |

## Workflow

1. **Session start**: `memory_load` loads top-15 scored memories
2. **During session**: `memory_search` finds relevant memories on demand
3. **Learning captured**: `memory_add` + optional `memory_add_edge`
4. **Session end**: `memory_decay` reinforces used memories, decays unused
<!-- memorygraph:end -->
