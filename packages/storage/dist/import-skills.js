...
function detectCategory(name) {
    ...
    if (name.startsWith('seo-'))
        return 'SEO';
    if (name.startsWith('pixarts'))
        return 'Pixarts';
    if (name.includes('cro') || name.includes('marketing') || name.includes('strategy'))
        return 'Marketing';
    return 'Other';
}
...
export function importSkills(workspacePath) {
    ...
    walkDir(agentsSkillDir, (filePath, name) => {
        const content = fs.readFileSync(filePath, 'utf-8');
        const fm = parseFrontmatter(content);
        const type = isLifecycleSkill(name) ? 'lifecycle' : 'process';
        skills.push({
            name: fm.name || name,
...
function isLifecycleSkill(name) {
...
//# sourceMappingURL=import-skills.js.map