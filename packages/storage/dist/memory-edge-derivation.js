...
const contextSimilar = (a, b) => {
    ...
    if (na === nb)
        return true;
    if (na.length >= 30 && nb.includes(na.slice(0, 30)))
        return true;
    if (nb.length >= 30 && na.includes(nb.slice(0, 30)))
        return true;
    return false;
};
const opposite = (a, b) => {
...
export function deriveEdgeCandidates(subject, candidates) {
    const out = [];
...
//# sourceMappingURL=memory-edge-derivation.js.map