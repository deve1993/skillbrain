...
export function parseLearningsFile(content, filePath) {
    const learnings = [];
...
function parseBlock(block, filePath, skill) {
...
function extractSkillName(filePath) {
    ...
    if (learningsIdx > 0)
        return parts[learningsIdx - 1];
    return 'unknown';
}
function findLearningsFiles(workspacePath) {
    const files = [];
    const searchDirs = [
...
export function migrate(workspacePath) {
