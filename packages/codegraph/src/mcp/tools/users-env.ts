...
function withUsersEnvStore<T>(repoPath: string, fn: (store: UsersEnvStore) => T): T {
...
function projectVarNames(repoPath: string, project: string): string[] {
  const db = openDb(repoPath)
  try {
...
const requireUser = (ctx: ToolContext) => {
  if (!ctx.userId) {
    throw new Error(
      'user_env tools require an authenticated MCP session. ' +
...
export function registerUserEnvTools(server: McpServer, ctx: ToolContext): void {
  // --- user_env_list ---
  server.tool(
    'user_env_list',
    'List the names + metadata of credentials/configs in YOUR personal master.env. Values are NOT returned — use user_env_get for that. Use this for capability checks ("do I have X?") without revealing secrets.',
    {
  ...
  server.tool(
    'user_env_get',
    'Decrypt and return the value of a single credential from YOUR master.env. Bumps last_used_at. If a `project` is provided AND that project also has the same var, the response includes a `conflict: true` flag — in that case ASK the user which to use before proceeding.',
    {
  ...
  server.tool(
    'user_env_available',
    'Quick boolean check: does YOUR master.env have credentials for a given service or var? Useful as a precondition before attempting to use a tool/integration.',
    {
  ...
  server.tool(
    'user_env_set',
    'Save (or update) a credential/config in YOUR master.env. Use after the user provides a new key so future sessions can find it via user_env_get.',
    {
      varName: z.string(),
      value: z.string(),
    ...
    async ({ varName, value, category, service, description, isSecret, repo }) => {
      try {
        ...
        const saved = withUsersEnvStore(resolved.path, (s) =>
          s.setEnv(userId, varName, value, { category, service, description, isSecret, source: 'mcp' }),
        )
        return {
          content: [{
            type: 'text',
