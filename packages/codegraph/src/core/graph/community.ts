 ...
 */
export function detectCommunities(store: GraphStore): number {
  ...
  if (symbolNodes.length === 0) return 0
  ...
  const allEdges = store.rawQuery(
    "SELECT source_id, target_id FROM edges WHERE type IN ('CALLS', 'IMPORTS')",
  ...
  for (const e of allEdges) {
    ...
    if (srcAdj) srcAdj.set(e.target_id, (srcAdj.get(e.target_id) || 0) + 1)
    const tgtAdj = adj.get(e.target_id)
    if (tgtAdj) tgtAdj.set(e.source_id, (tgtAdj.get(e.source_id) || 0) + 1)
  }
  ...
  for (const node of symbolNodes) {
    ...
    if (!communityMembers.has(com)) communityMembers.set(com, [])
...
function heuristicLabel(members: GraphNode[]): string {
