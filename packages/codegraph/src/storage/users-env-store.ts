...
export class UsersEnvStore {
  ...
  setEnv(userId: string, varName: string, value: string, opts: UserEnvSetOptions = {}): UserEnvVar {
    ...
    const existing = this.db.prepare(
      'SELECT id, created_at FROM user_env_vars WHERE user_id = ? AND var_name = ?'
    ).get(userId, varName) as { id: string; created_at: string } | undefined

    const id = existing?.id || randomId()
    const createdAt = existing?.created_at || now

    const category: EnvCategory = opts.category ?? 'api_key'
    ...
    `).run(
      id, userId, varName, enc.ciphertext, enc.iv, enc.authTag,
      category, opts.service ?? null, isSecret ? 1 : 0,
      opts.description ?? null, opts.source ?? 'manual',
      createdAt, now,
    )
  ...
  getEnv(userId: string, varName: string): string | undefined {
    const row = this.db.prepare(
      'SELECT id, encrypted_value, iv, auth_tag FROM user_env_vars WHERE user_id = ? AND var_name = ?'
    ).get(userId, varName) as { id: string; encrypted_value: string; iv: string; auth_tag: string } | undefined
  ...
  getAllEnv(userId: string, opts: UserEnvListOptions = {}): Record<string, string> {
    ...
    for (const meta of rows) {
      const row = this.db.prepare(
        'SELECT encrypted_value, iv, auth_tag FROM user_env_vars WHERE id = ?'
      ).get(meta.id) as { encrypted_value: string; iv: string; auth_tag: string } | undefined
      if (!row) continue
      try {
  ...
  listEnv(userId: string, opts: UserEnvListOptions = {}): UserEnvVar[] {
    ...
    if (opts.service)  { conds.push('service = ?');  params.push(opts.service) }

    const rows = this.db.prepare(
      `SELECT id, user_id, var_name, category, service, is_secret, description, source, last_used_at, created_at, updated_at
       FROM user_env_vars
       WHERE ${conds.join(' AND ')}
       ORDER BY category, service, var_name`
    ).all(...params) as any[]
  ...
  hasEnv(userId: string, varName: string): boolean {
    const row = this.db.prepare(
      'SELECT 1 FROM user_env_vars WHERE user_id = ? AND var_name = ?'
    ).get(userId, varName)
    return !!row
  }
  ...
  capability(userId: string): {
    ...
    const rows = this.db.prepare(
      `SELECT service, category FROM user_env_vars WHERE user_id = ?`
    ).all(userId) as { service: string | null; category: EnvCategory }[]

    const services = new Set<string>()
    const categories: Record<EnvCategory, number> = {
  ...
  conflictsWith(userId: string, projectVarNames: string[]): string[] {
    if (projectVarNames.length === 0) return []
    const placeholders = projectVarNames.map(() => '?').join(',')
    const rows = this.db.prepare(
      `SELECT var_name FROM user_env_vars WHERE user_id = ? AND var_name IN (${placeholders})`
    ).all(userId, ...projectVarNames) as { var_name: string }[]
    return rows.map((r) => r.var_name)
  }

  deleteEnv(userId: string, varName: string): boolean {
    const result = this.db.prepare(
      'DELETE FROM user_env_vars WHERE user_id = ? AND var_name = ?'
    ).run(userId, varName)
   ...
   */
  importEnv(userId: string, envContent: string, opts: UserEnvSetOptions = {}): { saved: number; errors: string[] } {
    let saved = 0
    const errors: string[] = []
  ...
  private getMeta(userId: string, varName: string): UserEnvVar | undefined {
    const row = this.db.prepare(
      `SELECT id, user_id, var_name, category, service, is_secret, description, source, last_used_at, created_at, updated_at
       FROM user_env_vars WHERE user_id = ? AND var_name = ?`
    ).get(userId, varName) as any
    return row ? this.rowToMeta(row) : undefined
  }

  private rowToMeta(row: any): UserEnvVar {
    return {
      id: row.id,
      userId: row.user_id,
