...
import { MEMORY_SCHEMA_SQL, MEMORY_FTS_SQL, SESSION_LOG_SQL, SESSION_LOG_MIGRATE_SQL, NOTIFICATIONS_SQL, SKILLS_SCHEMA_SQL, SKILLS_FTS_SQL, PROJECTS_SCHEMA_SQL, PROJECTS_MIGRATE_SQL, PROJECT_ENV_SCHEMA_SQL } from './memory-schema.js'
...
export function openDb(repoPath: string): Database.Database {
  ...
  // Safe migration for projects — adds team fields
  for (const stmt of PROJECTS_MIGRATE_SQL.split(';').filter(s => s.trim())) {
    try { db.exec(stmt) } catch { /* column already exists */ }
  }
  db.exec(PROJECT_ENV_SCHEMA_SQL)
