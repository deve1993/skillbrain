...
const memoryTypes = ['Fact', 'Preference', 'Decision', 'Pattern', 'AntiPattern', 'BugFix', 'Goal', 'Todo'];
const memoryEdgeTypes = ['RelatedTo', 'Updates', 'Contradicts', 'CausedBy', 'PartOf'];
export function registerMemoryTools(server, _ctx) {
    // --- Tool: memory_add ---
    server.tool('memory_add', 'Add a new memory (learning, fact, decision, pattern, etc.) to the Memory Graph', {
        ...
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found. Run codegraph_list_repos.' }] };
        const memory = withMemoryStore(resolved.path, (store) => {
            ...
            const contradictionWarnings = contradictions.map((c) => `⚠️ Potential contradiction with ${c.id}: "${c.context.slice(0, 80)}..."`);
    ...
    // --- Tool: memory_search ---
    server.tool('memory_search', 'Search memories by semantic query (full-text search across context, problem, solution, reason)', {
    ...
    // --- Tool: memory_query ---
    server.tool('memory_query', 'Query memories by type, project, skill, confidence, tags, or status', {
        ...
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found.' }] };
        const results = withMemoryStore(resolved.path, (store) => store.query({ type, project, skill, minConfidence, tags, status, limit }));
    ...
    // --- Tool: memory_load ---
    server.tool('memory_load', 'Load top-scored memories for current session (replaces load-learnings). Uses scoring algorithm: confidence, scope match, recency, skill relevance', {
        ...
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found.' }] };
        const results = withMemoryStore(resolved.path, (store) => store.scored(project, activeSkills, limit));
        const byType = {};
        const formatted = results.map((r) => {
    ...
    server.tool('memory_add_edge', 'Create a relationship between two memories (RelatedTo, Updates, Contradicts, CausedBy, PartOf)', {
        ...
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found.' }] };
        const edge = withMemoryStore(resolved.path, (store) => store.addEdge(sourceId, targetId, type, reason));
        return {
            content: [{ type: 'text', text: `✅ Edge created: ${sourceId} --[${type}]--> ${targetId}` }],
        };
    });
    // --- Tool: memory_stats ---
    server.tool('memory_stats', 'Get Memory Graph statistics: counts by type, status, and edge count', {
        repo: z.string().optional(),
    }, async ({ repo }) => {
        const resolved = resolveMemoryRepo(repo);
        if (!resolved)
    ...
    // --- Tool: memory_decay ---
    server.tool('memory_decay', 'Apply decay cycle: reinforce validated memories, decay unused ones, mark stale as pending-review/deprecated', {
        ...
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found.' }] };
        const result = withMemoryStore(resolved.path, (store) => store.applyDecay(validatedIds, sessionDate));
        return {
            content: [{
                    type: 'text',
    ...
    // --- Tool: memory_suggest ---
    server.tool('memory_suggest', 'Call this at the end of a task. Returns a template for Claude to propose 1-3 memory candidates based on what was done. Claude fills in the details and calls memory_add for each approved one.', {
