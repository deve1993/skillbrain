    ...
    server.tool('design_system_scan', 'Scan project design tokens and auto-populate the design system. Pass file contents directly via cssContent/tailwindContent/tokensContent (recommended — avoids server filesystem access), or provide workspacePath for local server setups. If no conflicts, saves immediately. If conflicts exist, saves as pending for dashboard review at memory.fl1.it/#/design-systems.', {
        ...
        try {
            ...
            if (cssContent || tailwindContent || tokensContent) {
                ...
                if (tailwindContent)
                    sources.push({ source: 'tailwind', path: 'tailwind.config', tokens: parseTailwindConfigFromContent(tailwindContent) });
                if (cssContent)
                    sources.push({ source: 'css', path: 'globals.css', tokens: parseCSSVariablesFromContent(cssContent) });
                if (tokensContent)
                    sources.push({ source: 'tokens_json', path: 'tokens.json', tokens: parseTokensJsonFromContent(tokensContent) });
            }
