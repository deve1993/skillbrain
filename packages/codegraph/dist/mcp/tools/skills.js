...
function withSkillsStore(repoPath, fn) {
...
export function registerSkillTools(server, ctx) {
    ...
    // --- Tool: skill_update ---
    server.tool('skill_update', 'Update the content of an existing skill in the database. Use after generating an improved version based on recent learnings.', {
    ...
    // --- Tool: agent_list ---
    server.tool('agent_list', 'List all available agents with their model, effort level, and purpose', { repo: z.string().optional() }, async ({ repo }) => {
        const resolved = resolveMemoryRepo(repo);
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found.' }] };
    ...
    // --- Tool: agent_read ---
    server.tool('agent_read', 'Read the full prompt of an agent', {
        ...
        if (!agent)
            return { content: [{ type: 'text', text: `Agent "${name}" not found. Use agent_list.` }] };
        return { content: [{ type: 'text', text: agent.content }] };
    });
    // --- Tool: command_list ---
    server.tool('command_list', 'List all available slash commands', { repo: z.string().optional() }, async ({ repo }) => {
        const resolved = resolveMemoryRepo(repo);
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found.' }] };
    ...
    // --- Tool: command_read ---
    server.tool('command_read', 'Read the full content of a slash command', {
        ...
        if (!cmd)
            return { content: [{ type: 'text', text: `Command "${name}" not found. Use command_list.` }] };
    ...
    // --- Tool: skill_stats ---
    server.tool('skill_stats', 'Get statistics about all skills, agents, and commands in the system', { repo: z.string().optional() }, async ({ repo }) => {
        const resolved = resolveMemoryRepo(repo);
        if (!resolved)
            return { content: [{ type: 'text', text: 'Repository not found.' }] };
