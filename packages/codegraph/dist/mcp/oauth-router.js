...
function clientAuth(req) {
    ...
    if (basic)
        return { id: basic.id, secret: basic.secret };
    const body = (req.body || {});
    return {
...
function redirectUriMatches(requested, registered) {
    ...
    for (const reg of registered) {
        ...
        if (reqUrl.protocol !== regUrl.protocol)
            continue;
        if (reqUrl.pathname !== regUrl.pathname)
            continue;
        const isLoopback = (h) => h === 'localhost' || h === '127.0.0.1' || h === '[::1]';
        if (reqUrl.hostname === regUrl.hostname)
            return true;
        if (isLoopback(reqUrl.hostname) && isLoopback(regUrl.hostname))
            return true;
    }
    return false;
}
export function createOAuthRouter(deps) {
    ...
    router.post('/oauth/register', express.json(), (req, res) => {
        ...
        if (redirectUris.length === 0) {
            jsonError(res, 400, 'invalid_client_metadata', 'redirect_uris required');
            return;
        }
        ...
        try {
            ...
            jsonError(res, 500, 'server_error', err.message);
        }
        finally {
            closeDb(db);
        }
    ...
    router.get('/oauth/authorize', (req, res) => {
        ...
        if (!clientId || !redirectUri || !responseType || !codeChallenge) {
            jsonError(res, 400, 'invalid_request', 'missing required params');
            return;
        }
        if (responseType !== 'code') {
            jsonError(res, 400, 'unsupported_response_type', 'only "code" supported');
            return;
        }
        if (codeChallengeMethod !== 'S256') {
            jsonError(res, 400, 'invalid_request', 'only S256 PKCE supported');
            return;
        }
        const db = openDb(skillbrainRoot);
        const store = new OAuthStore(db);
        ...
        const registeredRedirects = JSON.parse(client.redirect_uris || '[]');
        if (!redirectUriMatches(redirectUri, registeredRedirects)) {
            closeDb(db);
            jsonError(res, 400, 'invalid_request', 'redirect_uri mismatch');
            return;
        }
        ...
        if (!userId) {
            ...
            res.redirect(`/?return_to=${encodeURIComponent(returnTo)}`);
            return;
        }
    ...
    router.get('/oauth/authorize/continue', (req, res) => {
        ...
        const userId = getUserIdFromRequest(req);
        if (!requestId) {
            jsonError(res, 400, 'invalid_request', 'missing request_id');
            return;
        }
        if (!userId) {
            const returnTo = `/oauth/authorize/continue?request_id=${requestId}`;
        ...
        try {
            ...
            const row = db.prepare('SELECT * FROM oauth_auth_requests WHERE id = ?').get(requestId);
            if (!row) {
                jsonError(res, 400, 'invalid_request', 'unknown or expired request');
                return;
            }
            const client = store.getClient(row.client_id);
            if (!client) {
                jsonError(res, 400, 'invalid_client', 'client disappeared');
                return;
            }
            res.type('html').send(renderConsentPage({
    ...
    router.post('/oauth/authorize/consent', express.urlencoded({ extended: false }), (req, res) => {
        ...
        try {
            ...
            if (!authReq) {
                jsonError(res, 400, 'invalid_request', 'request expired or already consumed');
                return;
            }
    ...
    router.post('/oauth/token', express.urlencoded({ extended: false }), express.json(), (req, res) => {
        ...
        if (!auth.id) {
            jsonError(res, 400, 'invalid_client', 'client_id required');
            return;
        }
        const db = openDb(skillbrainRoot);
        try {
            ...
            if (!client) {
                jsonError(res, 401, 'invalid_client', 'unknown client');
                return;
            }
            ...
            if (isConfidential) {
                if (!auth.secret || !store.verifyClientSecret(auth.id, auth.secret)) {
                    ...
                    jsonError(res, 401, 'invalid_client', 'client authentication failed');
                    return;
                }
            }
            if (grantType === 'authorization_code') {
                ...
                if (!code || !codeVerifier) {
                    jsonError(res, 400, 'invalid_request', 'code and code_verifier required');
                    return;
                }
            ...
            if (grantType === 'refresh_token') {
                ...
                if (!refreshToken) {
                    jsonError(res, 400, 'invalid_request', 'refresh_token required');
                    return;
                }
                const stored = store.lookupToken(refreshToken);
                if (!stored || stored.token_type !== 'refresh' || stored.client_id !== auth.id) {
                    jsonError(res, 400, 'invalid_grant', 'invalid refresh token');
                    return;
                }
 ...
 */
export function verifyOAuthBearer(skillbrainRoot, token) {
    const db = openDb(skillbrainRoot);
    try {
        const store = new OAuthStore(db);
        const stored = store.lookupToken(token);
        if (!stored || stored.token_type !== 'access')
            return null;
