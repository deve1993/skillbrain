...
export function extractCalls(sourceFile, relativePath) {
    const edges = [];
    const seen = new Set();
    ...
    for (const call of callExprs) {
        ...
        if (!calledName || calledName.length > 100)
            continue;
    ...
    for (const jsx of jsxElements) {
        ...
        if (!tagName || tagName[0] !== tagName[0].toUpperCase() || tagName[0] === tagName[0].toLowerCase())
            continue;
        if (tagName.length > 100 || tagName.includes('.'))
            continue;
