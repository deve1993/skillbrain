...
import { fileHash, edgeId } from '../../utils/hash.js';
import { info, dim } from '../../utils/logger.js';
import path from 'node:path';
export function buildGraph(project, store, repoPath, incremental) {
    ...
    if (incremental) {
        const existingNodes = store.rawQuery('SELECT id FROM nodes');
        for (const n of existingNodes)
            validNodeIds.add(n.id);
    }
    const validEdges = allEdges.filter((e) => validNodeIds.has(e.sourceId) && validNodeIds.has(e.targetId));
    ...
    const invalidCallEdges = allEdges.filter((e) => e.type === 'CALLS' && validNodeIds.has(e.sourceId) && !validNodeIds.has(e.targetId));
