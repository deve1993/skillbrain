...
async function openEditProjectModal(name) {
  const meta = await fetch(`${API}/api/projects-meta/${encodeURIComponent(name)}`).then(r => r.ok ? r.json() : null).catch(() => null) || { name }
  ...
  const stackStr = (meta.stack || []).join(', ')
  const domainsExtraStr = (meta.domainsExtra || []).join(', ')
...
function editField(key, label, value, placeholder, type = 'text') {
...
function editSelect(key, label, value, options) {
  return `<div>
    <label style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px">${label}</label>
...
async function saveProject(event, name) {
  ...
  const r = await fetch(`${API}/api/projects-meta/${encodeURIComponent(name)}`, {
    method: 'PUT', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(fields),
  })
  if (r.ok) {
    closeEditModal()
    openProjectDetail(name) // reload
  } else {
    alert('Save failed')
  }
  return false
